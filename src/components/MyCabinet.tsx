...
import { EABot, UserProfile, Purchase, toPurchase } from '../types';
import { supabase, handleSupabaseError } from '../supabase';
import { simulateDownloadFile } from '../utils/simulation';
import { ShoppingBag, Key, Download, Code, ExternalLink, ClipboardCopy, Terminal, FileCode, Activity } from 'lucide-react';
...
export default function MyCabinet({ userProfile, bots, onSelectBot }: MyCabinetProps) {
  ...
  const [activeCodeBotId, setActiveCode]  = useState<string | null>(null);
  const [activeCodeFile, setCodeFile]     = useState<string | null>(null);

  useEffect(() => { if (userProfile) loadPurchases(); }, [userProfile?.id]);

  const loadPurchases = async () => {
    if (!userProfile) return;
    setLoading(true);
    const { data, error } = await supabase
      .from('purchases').select('*').eq('buyer_id', userProfile.id).order('purchase_date', { ascending: false });
    if (error) handleSupabaseError(error, 'loadPurchases');
    else setPurchases((data ?? []).map(toPurchase));
    setLoading(false);
  };
  ...
  const toggleCode = (purchase: Purchase, bot: EABot) => {
    if (activeCodeBotId === bot.id) { setActiveCode(null); setCodeFile(null); return; }
    setActiveCode(bot.id);
    setCodeFile(simulateDownloadFile(bot.name, bot.sourceFileName, purchase.licenseKey));
  };
  ...
  if (!userProfile) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center animate-fade-in">
        ...
        <p className="text-slate-500 text-sm">Sign in to view your licensed Expert Advisors.</p>
      </div>
    );
  }
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">Manage licenses · Download source · Inspect performance</p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="card-ink rounded-2xl p-16 text-center animate-fade-in">
          ...
          <p className="text-xs text-slate-500 font-mono leading-relaxed max-w-md mx-auto">
            Acquire licenses from the marketplace to sync your portfolio here.
          </p>
        </div>
      ) : (
        <div className="space-y-5 stagger-children">
          {purchases.map(purchase => {
            const bot = bots.find(b => b.id === purchase.botId);
            return (
              <div key={purchase.id} className="card-ink shimmer-sweep rounded-2xl p-5 sm:p-6 animate-fade-in-up hover:-translate-y-0.5 transition-transform duration-200">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 font-mono mt-0.5">
                        Licensed {purchase.purchaseDate ? new Date(purchase.purchaseDate).toLocaleDateString() : 'Recently'}
                        {bot && ` · ${bot.platform} · ${bot.strategy}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 bg-black/40 border border-cyan-500/12 rounded-xl px-3 py-2 font-mono text-xs min-w-0 max-w-xs">
                    ...
                    <button onClick={() => copyKey(purchase.licenseKey)}
                      className="p-1.5 bg-cyan-500/8 hover:bg-cyan-500/18 text-slate-400 hover:text-cyan-400 rounded-lg border border-cyan-500/15 transition-all" title="Copy">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {bot && (
                      <button onClick={() => onSelectBot(bot)}
                    ...
                    {bot && (
                      <button onClick={() => toggleCode(purchase, bot)}
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all ${
                          activeCodeBotId === bot.id
                            ? 'bg-violet-500/15 text-violet-400 border border-violet-500/30'
                            : 'btn-cyan'
                        }`}>
                        <Code className="w-3.5 h-3.5" />
                        {activeCodeBotId === bot.id ? 'Minimize' : 'Get Source'}
                      </button>
                    )}
                  </div>
                </div>
                ...
                {bot && (
                  <div className="mt-4 pt-4 border-t border-cyan-500/8 flex flex-wrap gap-4 font-mono text-xs">
                    <div>
                      ...
                      <span className="text-emerald-400 font-black">+{bot.monthlyProfit}% <span className="text-slate-600 font-normal">/mo</span></span>
                    </div>
                ...
                {activeCodeBotId === purchase.botId && activeCodeFile && (
                  <div className="mt-5 bg-black/60 border border-cyan-500/12 rounded-xl p-4 font-mono text-[11px] relative animate-fade-in">
                    ...
                    <h4 className="text-xs text-slate-400 font-bold mb-3 flex items-center gap-1.5">
                      <FileCode className="w-3.5 h-3.5 text-cyan-400" />{bot?.sourceFileName}
                    </h4>
