...
import { supabase, signInWithGoogle, logoutUser } from '../supabase';
import { UserProfile } from '../types';
import { LogOut, LogIn, Coins, Activity, ShoppingBag, Award, ShieldAlert, Zap } from 'lucide-react';
...
export default function Navigation({ userProfile, activeTab, setActiveTab, loadingAuth, onAddFunds }: NavigationProps) {
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <nav className={`transition-all duration-300 ${
        scrolled ? 'bg-[#030712]/95 backdrop-blur-xl border-b border-cyan-500/10 shadow-lg shadow-black/60'
                 : 'bg-[#030712]/80 backdrop-blur-lg border-b border-white/5'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            {/* Logo */}
            <button onClick={() => setActiveTab('marketplace')} className="flex items-center gap-3 group">
              ...
              <div className="text-left">
                <div className="font-black text-base tracking-tight text-white leading-none">EA<span className="text-cyan-400">Market</span></div>
                <div className="text-[9px] font-mono text-slate-500 tracking-[0.15em] uppercase leading-none mt-0.5">Supabase · Algo Hub</div>
              </div>
            </button>
              ...
              {navLinks.map(link => (
                <button key={link.id} onClick={() => setActiveTab(link.id)}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-[13px] font-semibold tracking-wide transition-all duration-200 ${
                    activeTab === link.id
                      ? (link.danger ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'nav-active')
            ...
            <div className="flex items-center gap-3">
              {loadingAuth ? (
                ...
                <div className="flex items-center gap-3">
                  <button onClick={onAddFunds} title="Click to add $1,000"
                    ...
                    <div>
                      ...
                      <div className="text-sm font-mono font-bold text-cyan-400 leading-tight">${userProfile.balance.toLocaleString()}</div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 bg-white/3 border border-white/8 rounded-xl p-1.5 pl-3">
                    ...
                    <button onClick={() => logoutUser()} className="p-1.5 text-slate-500 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all" title="Sign Out">
                ...
                <button onClick={() => signInWithGoogle()}
                  ...
                  <LogIn className="w-4 h-4 stroke-[2.5]" /> Connect Google
                </button>
              )}

              <button className="md:hidden p-2 text-slate-400 hover:text-cyan-400 transition-colors" onClick={() => setMobileOpen(!mobileOpen)}>
        ...
        {mobileOpen && (
          <div className="md:hidden border-t border-cyan-500/10 bg-[#030712]/95 backdrop-blur-xl animate-fade-in">
            <div className="px-4 py-3 flex flex-col gap-1">
              {navLinks.map(link => (
                <button key={link.id} onClick={() => { setActiveTab(link.id); setMobileOpen(false); }}
                  className={`flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-semibold text-left transition-all ${
                    activeTab === link.id ? (link.danger ? 'bg-red-500/10 text-red-400' : 'nav-active') : 'text-slate-400 hover:text-white hover:bg-white/5'
                  }`}>
                  {link.icon}{link.label}
                </button>
              ))}
